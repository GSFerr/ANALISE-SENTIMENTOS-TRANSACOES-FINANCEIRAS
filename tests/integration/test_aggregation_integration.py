...
def aggregate_data(spark: SparkSession, silver_path: str, gold_path: str):
    """
    Agrega dados da camada Silver para criar a camada Gold (data mart).

    Args:
        spark (SparkSession): A sessão Spark.
        silver_path (str): O caminho para a pasta de dados da camada Silver.
        gold_path (str): O caminho para a pasta de destino da camada Gold.
    ...
    logging.info(f"Lendo dados da camada Silver do caminho: {silver_path}")
    try:
    ...
    logging.info(f"Agregação concluída. Salvando dados na camada Gold no caminho: {gold_path}")
...
if __name__ == "__main__":
    ...
    if spark_session:
        # Caminhos fixos para a execução padrão do pipeline
        silver_path_main = os.path.join("data", "processed", "processed_data.parquet")
        gold_path_main = os.path.join("data", "mart", "merchant_sentiment_agg.parquet")

        data_mart_df = aggregate_data(spark_session, silver_path_main, gold_path_main)
        if data_mart_df:
